...
# Directly set environment variables (instead of loading from .env)
$sourcePath = "G:/My Drive/Notes/Developer portfolio"
$destinationPath = "C:/Users/User/Documents/hugo/portfolio/content/portfolio"
$myrepo = "git@github.com:Dev-Pradeep-NS/portfolio.git"
$nodeCommand = "node"

# Validate environment variables
if (-not $sourcePath) {
    Write-Error "SOURCE_PATH is not defined."
    exit 1
}

if (-not $destinationPath) {
    Write-Error "DESTINATION_PATH is not defined."
    exit 1
}

if (-not $myrepo) {
    Write-Error "REPO_URL is not defined."
    exit 1
}

if (-not $nodeCommand) {
    Write-Error "NODE_COMMAND is not defined."
    exit 1
}
...
$commitMessage = "New Addition on $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"
